...
S_F=4
S_A=17
GPIO.setup(S_F,GPIO.IN, pull_up_down=GPIO.PUD_DOWN)   #S_FECHADO
GPIO.setup(S_A,GPIO.IN,pull_up_down=GPIO.PUD_DOWN)  #S_ABERTO
...
def main():
 ...
 #---------------------------------   
    while True:
        ...
        #print tag,",",cmd_a,",",cmd_f,",",GPIO.input(S_F),",",GPIO.input(17),",",GPIO.input(27),",",GPIO.input(22),",",GPIO.input(10),",",falha
        if not cmd_a and tag and GPIO.input(S_F) and GPIO.input(22) and not falha:
        ...
        if cmd_a and GPIO.input(S_F) and not falha:
            falha=1
        ...
        if cmd_a and not GPIO.input(S_F) and not falha:
            abrindo=1
        ...
        if cmd_f and not GPIO.input(17) and not var[2] and not falha:
            ...
            time.sleep(var[1])
        #falha no fechamento
        if fechando and not GPIO.input(S_F) and not var[2] and not falha:
            falha=1
        ...
        #confirmado fechado
        if (cmd_ff or cmd_f) and GPIO.input(S_F):            
            fechando=0
            cmd_f=0
            cmd_ff=0
