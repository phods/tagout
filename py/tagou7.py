 ...
 #---------------------------------   
    while True:
        ...
        if cmd_a and not GPIO.input(S1_FEC) and not falha :
            abrindo=1
            print "Portao 1 - Aguarda",portao1[1],"s P/CHEGAR NO FIM"
            log("","Portao 1 - Abrindo - Aguardando Fim de Curso")            

        if abrindo and not GPIO.input(S1_ABE) and not falha and temp(0,temp1,portao1[1]):
            print "Portao 1 - FALHA NA ABERTURA ",portao1[1],"s E NAO ABRIU"
        ...
        if GPIO.input(S1_LAC2) and not GPIO.input(S1_FEC) and not laco2 and not falha:
            laco2=1
            print "Portao 1 - LACO 2 ATIVADO"
            log("","Portao 1 - Laço 2 - Ativado")
        #CASO O LAÇO JA TENHA SIDO ATIVDADO/s_aberto/infra ok
        if not cmd_f and laco2 and GPIO.input(S1_ABE) and GPIO.input(S1_INF) and not falha and not portao1[2]:
            #comando motor 1 segundo
        ...
        #falha no fechamento
        if cmd_f and GPIO.input(S1_ABE) and not falha and temp(0,temp1,10):
        ...
        if cmd_f and not GPIO.input(S1_ABE) and not portao1[2] and not falha:
            fechando =1
            print "Portao 1 - Fechando",portao1[1],"s P/CHEGAR NO FIM"
            log("","Portao 1 - Fechando - Aguardando Fim de Curso")
            
        #falha no fechamento
        if fechando and not GPIO.input(S1_FEC) and not portao1[2] and not falha and temp(0,temp1,portao1[1]):
                falha=1
        ...
        if not tag and not cmd_ff  and not GPIO.input(S1_LAC1) and not GPIO.input(S1_LAC2)and GPIO.input(S1_ABE) and GPIO.input(S1_INF) and not falha and portao1[2]:
            ...
            if not tag and not cmd_ff  and not GPIO.input(S1_LAC1) and not GPIO.input(S1_LAC2)and GPIO.input(S1_ABE) and GPIO.input(S1_INF) and not falha and portao1[2]:
                    ...
                    print "Aguarda 10 segundos para descolar"
        #verifica se passaram 10 segundos e nao descolou           
        if cmd_ff and GPIO.input(S1_ABE) and portao1[2] and not falha and temp(0,temp1,10):
                print "Portao 1 - Falha no Acionamento Fila - Aguardou 10s o Sensor de Aberto Continua Acionado "
                log("","Portao 1 - Falha no Acionamento Fila - Aguardou 10s o Sensor de Aberto Continua Acionado")
                evento("Portao 1 - Falha de Acionamento Fila - Favor verifique o motor, caso tudo normal limpe a falha")
                falha =1   
                
        if cmd_ff and tag and GPIO.input(S1_LAC1) and portao1[2]:
        ...
        if abrindo2 and not GPIO.input(S2_ABE) and not falha2 and temp(0,temp2,portao2[1]):
            print "Portao 2 - FALHA NA ABERTURA ",portao2[1],"s E NAO ABRIU"
        ...
        #CASO O LAÇO JA TENHA SIDO ATIVDADO/s_aberto/infra ok
        if not cmd_f and not GPIO.input(S2_LAC1) and GPIO.input(S2_ABE) and GPIO.input(S2_INF) and not falha2 and not portao2[2]:
            #comando motor 1 segundo
        ...
        #fechando aguarda tempo para confirmar fechado
        if cmd2_f and not GPIO.input(S2_ABE) and not falha2 and not portao2[2]:
            fechando2 =1
            print "Portao 2 - Fechando",portao2[1],"s P/CHEGAR NO FIM"
            log("","Portao 2 - Fechando - Aguardando Fim de Curso")
            
        #falha no fechamento
        if fechando2 and not GPIO.input(S2_FEC) and not falha2 and not portao2[2] and temp(0,temp2,portao2[1]):
                falha2=1
                print "Portao 2 - FALHA NO FECHAMENTO ",portao2[1],"s E NAO FECHOU"
        ...
        if not cmd2_ff  and not GPIO.input(S2_LAC1) and GPIO.input(S2_ABE) and GPIO.input(S2_INF) and not falha2 and portao2[2]:
        ...
        if cmd2_ff and  GPIO.input(S2_LAC1) and portao2[2]:
