...
def comparar():
   ...
   #pesquisa no banco se existe o tag 
    ...
    for row in cur.fetchall():
        var_bd=row[0]
        print"var_bd  ",var_bd
    #le o ultimo tag que foi aberto o portao 
    cur.execute("SELECT tag_cmd FROM rfid_cmd order by ID desc limit 1")
    for row in cur.fetchall():
        var_cmd=row[0]
        print"var_cmd ",var_cmd   
        
    if(var_bd==var_temp and var_bd!=" " and var_temp!=var_cmd):
        ...
        try:
            cur.execute("INSERT INTO rfid_cmd (tag_cmd) VALUES (%s)", var_bd)

            print"inseridocmd"
            db.commit()
        except:
            print"NAO inseridocmd"
            db.rollback()
